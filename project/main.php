<?php
  ...
  // Description: This is the home page of the website where all public recipes and recipes owned by the current user if they are logged
  // in will be displayed, with a limit of 16 per page. The title, username, image, rating, and tags for the recipe are displayed for
  // each, and clicking on either the image or title will you to view the recipe in a pop-up modal window. If the user is not logged in,
  // no icons will be present on the recipe. If the user is logged in, they will have the option to duplicate any of the recipes, and the
  // options to edit or delete the recipes that they own. Page navigation is also located at the bottom of the page.
  ...
  if(isset($_GET['save-id'])) {
    ...
    $new_query = "INSERT INTO `yummy_recipes` (`username`, `title`, `image`, `description`, `ingredients`, `directions`, `minutes`, `servings`, `rating`, `category`, `private`, `tags`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $new_stmt = $pdo->prepare($new_query);
    $new_stmt->execute([$username, $dup_title, $dup_img_name, $dup_desc, $dup_ingredients, $dup_directions, $dup_time, $dup_servings, $dup_rating, $dup_category, 1, $dup_tags]);
  ...
  <body>
    ...
    <main>
      ...
      <div>
        <ul class="pagination">
          <?php
          // get the total number of recipes in the recipes table
          $query2 = "SELECT COUNT(*) FROM `yummy_recipes`";
          ...
          <li class="<?php if($pn <= 1){echo 'hidden';}?>">
            <a href="<?php if($pn <= 1){echo '#';} else {echo "?page=".($pn - 1);}?>"><i class="fas fa-angle-left"></i></a>
          </li>
          <li class="<?php if($pn >= $pages){echo 'hidden';}?>">
            <a href="<?php if($pn >= $pages){echo '#';} else {echo "?page=".($pn + 1);}?>"><i class="fas fa-angle-right"></i></a>
          </li>
          <li><a href="?page=<?php echo $pages;?>"><i class="fas fa-angle-double-right"></i></a></li>
        </ul>
      </div>
